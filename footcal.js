...
connection.query("SELECT CONCAT(players.first_name, ' ', players.last_name) as fullname, (SELECT COUNT(event_presences.confirmed) FROM event_presences JOIN events ON event_presences.eventID = events.event_ID JOIN club_event_types ON club_event_types.club_event_type_ID = events.event_type WHERE events.date < ? AND event_presences.playerID = players.player_ID AND event_presences.confirmed = 1 AND event_presences.unselected <> 1 AND club_event_types.event_type = 'training') as confirmedTrainings, (SELECT COUNT(event_presences.confirmed) FROM event_presences JOIN events ON event_presences.eventID = events.event_ID JOIN club_event_types ON club_event_types.club_event_type_ID = events.event_type WHERE events.date < ? AND event_presences.playerID = players.player_ID AND event_presences.confirmed = 1 AND event_presences.unselected <> 1 AND club_event_types.event_type <> 'training') as confirmedGames, (SELECT COUNT(event_presences.confirmed) FROM event_presences JOIN events ON event_presences.eventID = events.event_ID JOIN club_event_types ON club_event_types.club_event_type_ID = events.event_type WHERE events.date < ? AND event_presences.playerID = players.player_ID AND event_presences.declined = 1 AND event_presences.unselected <> 1 AND club_event_types.event_type = 'training') as declinedTrainings, (SELECT COUNT(event_presences.confirmed) FROM event_presences JOIN events ON event_presences.eventID = events.event_ID JOIN club_event_types ON club_event_types.club_event_type_ID = events.event_type WHERE events.date < ? AND event_presences.playerID = players.player_ID AND event_presences.declined = 1 AND event_presences.unselected <> 1 AND club_event_types.event_type <> 'training') as declinedGames FROM players WHERE players.teamID = ? ORDER BY confirmedTrainings, confirmedGames DESC", [req.params.date,req.params.date,req.params.date,req.params.date,req.params.teamid], function(err, rows, fields) {
  if (!err){
    console.log('The solution is: ', rows);
    res.send(JSON.stringify(rows));
  }else{
